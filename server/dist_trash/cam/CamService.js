...
const makerjs = __importStar(require("makerjs"));
class CamService {
    async generateVector(fileContent, options) {
        const { tool, format, feedrate } = options;
        const rootModel = { models: {} };
        // 1. Parse Input
        if (format === 'svg') {
            // Manual SVG Parsing
        ...
        const f = feedrate;
        // Header
        commands.push('; Generated by RevivedLaserweb4 (Vector)');
        commands.push('G90');
        commands.push('G21');
        commands.push(`F${f}`);
        commands.push(tool.type === 'laser' ? 'M3 S0' : 'M3');
        // Walker setup
        const walk = (model, offset = { x: 0, y: 0 }) => {
            if (model.paths) {
    ...
    async generateRaster(imagePath, options) {
        ...
        const lines = [];
        lines.push('; Generated by RevivedLaserweb4 (Raster)');
